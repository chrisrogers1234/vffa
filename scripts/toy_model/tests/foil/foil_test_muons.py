...
def energy_loss(material):
    ...
    p_list = [p for p in range(100, 101, 10)]
    dedz_list = []
    for p in p_list:
        particle = Particle.new_from_ke(p, 13)
        dedz = abs(material.energy_loss_dz(particle))
        dedz_list.append(dedz)
        print("p:", particle.p, "e:", particle.get_kinetic_energy(), "dedz:", dedz)
    fig = matplotlib.pyplot.figure(figsize=(20, 10))
    axes_dedzrho = fig.add_subplot(1, 1, 1, xscale="linear", yscale="linear", position=[0.1, 0.2, 0.0001, 0.8]) #matplotlib.axes.Axes(fig, [0.1, 0.1, 0.8, 0.8], sharex=axes_dedz)
    ...
    axes_dedz = fig.add_subplot(1, 1, 1, xscale="linear", yscale="linear", position=[0.2, 0.2, 0.7, 0.8])
...
def momentum_change(material, kinetic_energy, column_density):
    ...
    print("For material with column density", column_density, "dE/E", de_1/kinetic_energy, "dp/p", dp_over_p_1)
    return dp_over_p_1
